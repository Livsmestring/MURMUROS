...
def test_default_notes_match_spec():
...
def test_build_bassline_rejects_invalid_notes(bad_note):
    with pytest.raises(ValueError):
        generate_midi.build_bassline([bad_note])


def test_validate_notes_returns_input_unchanged():
    notes = [{'note': 60, 'velocity': 64, 'duration': 120}]
    assert generate_midi.validate_notes(notes) is notes
